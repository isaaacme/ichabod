...
export async function init() {
  ...
  if (framework === 'astro') {
    scaffoldAstro(webDir, lang);
  } else if (framework === 'nextjs') {
    scaffoldNextJs(webDir, lang);
  } else {
    scaffoldStatic(pbPublicDir, lang);
  }

  step('③ Starting PocketBase & setting up admin');
  const { token, pb } = await startAndSetup(pbDir, email, password);

  step('④ Creating schema');
  await createSiteSettingsCollection(token);
  await createPostsCollection(token);

  step('⑤ Seeding content');
  await seedSiteSettings(token, name, lang);
  await seedPosts(token);

  step('⑥ Discovering schema & writing config');
  await writeConfig(projectDir, token, framework);
  ...
  success(name, [
    ...
    ['Admin CMS',  'http://localhost:8090/admin/'],
