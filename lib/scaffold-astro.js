...
const ENV = (lang, pbPort) => `PUBLIC_PB_URL=http://localhost:${pbPort}
...
const LAYOUT_ASTRO = (pbPort) => `---
const { title, description = '', canonical = '', noIndex = false, active } = Astro.props;
const PB  = import.meta.env.PUBLIC_PB_URL  || 'http://localhost:${pbPort}';
...
export function scaffoldAstro(webDir, lang = 'en', pbPort = 8090) {
  write(path.join(webDir, 'package.json'), PACKAGE_JSON);
  write(path.join(webDir, 'astro.config.mjs'), ASTRO_CONFIG);
  write(path.join(webDir, '.env'), ENV(lang, pbPort));
  write(path.join(webDir, 'public', 'style.css'), GLOBAL_CSS);
  write(path.join(webDir, 'src', 'layouts', 'Layout.astro'), LAYOUT_ASTRO(pbPort));
  write(path.join(webDir, 'src', 'pages', 'index.astro'), INDEX_ASTRO(pbPort));
  write(path.join(webDir, 'src', 'pages', 'blog', 'index.astro'), BLOG_INDEX_ASTRO(pbPort));
  write(path.join(webDir, 'src', 'pages', 'blog', '[slug].astro'), BLOG_SLUG_ASTRO(pbPort));
