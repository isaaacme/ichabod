...
const LAYOUT_JS = (pbPort) => `import './globals.css';

const PB = process.env.NEXT_PUBLIC_PB_URL || 'http://localhost:${pbPort}';
...
const HOME_PAGE_JS = (pbPort) => `const PB = process.env.NEXT_PUBLIC_PB_URL || 'http://localhost:${pbPort}';
...
const BLOG_PAGE_JS = (pbPort) => `const PB = process.env.NEXT_PUBLIC_PB_URL || 'http://localhost:${pbPort}';
...
const POST_PAGE_JS = (pbPort) => `import { notFound } from 'next/navigation';
...
export function scaffoldNextJs(webDir, lang = 'en', pbPort = 8090) {
  ...
  write(path.join(webDir, 'src', 'app', 'layout.js'), LAYOUT_JS(pbPort));
  write(path.join(webDir, 'src', 'app', 'page.js'), HOME_PAGE_JS(pbPort));
  write(path.join(webDir, 'src', 'app', 'blog', 'page.js'), BLOG_PAGE_JS(pbPort));
  write(path.join(webDir, 'src', 'app', 'blog', '[slug]', 'page.js'), POST_PAGE_JS(pbPort));
