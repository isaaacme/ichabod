...
const NAV = (active) => `<nav>
    ...
    <div class="nav-links">
      ...
      <a href="http://localhost:8090/_/" target="_blank">Admin ↗</a>
    </div>
  </nav>`;

const FOOTER = `<footer>
    <span>Powered by <a href="https://pocketbase.io" target="_blank">PocketBase</a> + Ichabod</span>
    <span><a href="http://localhost:8090/_/" target="_blank">Admin UI</a> · <a href="http://localhost:8090/api/collections/posts/records" target="_blank">API</a></span>
  </footer>`;

const INDEX_HTML = (lang) => `<!DOCTYPE html>
<html lang="${lang}">
<head>
  <meta charset="UTF-8"/>
  ...
  <main>
    <section class="hero">
      ...
      <div class="hero-actions">
        ...
        <a class="btn btn-ghost" href="http://localhost:8090/_/" target="_blank">Open Admin UI</a>
      </div>
    </section>
...
const BLOG_INDEX_HTML = (lang) => `<!DOCTYPE html>
<html lang="${lang}">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
...
<body>
  ...
  <main>
    <div class="page-header">
      <h1>Blog</h1>
      <p>Manage posts in the <a href="http://localhost:8090/_/" target="_blank">PocketBase admin UI</a>.</p>
    </div>
...
const POST_HTML = (lang) => `<!DOCTYPE html>
<html lang="${lang}">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
...
export function scaffoldStatic(pbPublicDir, lang = 'en') {
  ...
  fs.writeFileSync(path.join(pbPublicDir, 'index.html'), INDEX_HTML(lang));
  fs.writeFileSync(path.join(pbPublicDir, 'blog', 'index.html'), BLOG_INDEX_HTML(lang));
  fs.writeFileSync(path.join(pbPublicDir, 'post.html'), POST_HTML(lang));
