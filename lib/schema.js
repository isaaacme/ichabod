import { httpPost, httpGet, PB_URL } from './setup.js';
...
export async function createSiteSettingsCollection(token) {
  spin('Creating site_settings collection');

  const existing = await httpGet(`${PB_URL}/api/collections/site_settings`, token);
  ...
  const res = await httpPost(`${PB_URL}/api/collections`, {
    name: 'site_settings',
    type: 'base',
    fields: [
  ...
  if (res.status !== 200 && res.status !== 201) {
    throw new Error(`Failed to create site_settings: ${JSON.stringify(res.body)}`);
  }
  log('ok', 'site_settings collection created');
}

export async function createPostsCollection(token) {
  spin('Creating posts collection');

  const existing = await httpGet(`${PB_URL}/api/collections/posts`, token);
  if (existing.status === 200) {
    stopSpin();
  ...
  if (res.status !== 200 && res.status !== 201) {
    throw new Error(`Failed to create posts collection: ${JSON.stringify(res.body)}`);
  }

  log('ok', 'posts collection created');
}

export async function discoverCollections(token) {
  const res = await httpGet(`${PB_URL}/api/collections?perPage=100`, token);
