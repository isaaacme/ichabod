import { httpPost, PB_URL } from './setup.js';
...
<h2>Key URLs</h2>
<ul>
  <li><strong>Your site:</strong> <a href="http://localhost:8090">http://localhost:8090</a></li>
  <li><strong>Blog:</strong> <a href="http://localhost:8090/blog/">http://localhost:8090/blog/</a></li>
  <li><strong>PocketBase Admin UI:</strong> <a href="http://localhost:8090/_/">http://localhost:8090/_/</a></li>
  <li><strong>API:</strong> <a href="http://localhost:8090/api/collections/posts/records">http://localhost:8090/api/collections/posts/records</a></li>
</ul>

<h2>How to manage content</h2>
<p>Open the <a href="http://localhost:8090/_/">PocketBase Admin UI</a>, log in with your credentials, and click on the <strong>posts</strong> collection to add, edit, or delete content. Changes are live instantly — no rebuilds needed.</p>
...
The site fetches data from a PocketBase API at http://localhost:8090.
...
2. Create a new file pb/pb_public/projects/index.html that lists all projects
   fetched from http://localhost:8090/api/collections/projects/records.
...
<ul>
  ...
  <li>Tell the LLM that your API base URL is <code>http://localhost:8090</code> (or your production domain).</li>
...
export async function seedSiteSettings(token, siteName, lang) {
  spin('Seeding site settings');
  const res = await httpPost(`${PB_URL}/api/collections/site_settings/records`, {
    site_title:       siteName,
    site_description: '',
    lang,
    robots:           'index,follow',
...
export async function seedPosts(token) {
  spin('Seeding starter posts');

  for (const post of [POST_1, POST_2]) {
    const res = await httpPost(`${PB_URL}/api/collections/posts/records`, post, token);
    if (res.status !== 200 && res.status !== 201) {
