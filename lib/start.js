...
export async function start() {
  ...
  if (fs.existsSync(cfgPath)) {
    try {
      ...
      framework = cfg.framework ?? 'static';
      pbPort = cfg.pbPort ?? 8090;
    } catch {}
  }

  await checkPortFree(pbPort);
  ...
  const sitePort  = FRAMEWORK_PORTS[framework] ?? pbPort;
  ...
  console.log(col('Admin CMS')  + `http://localhost:${pbPort}/admin/\x1b[0m`);
  console.log(col('PB Admin')   + `http://localhost:${pbPort}/_/\x1b[0m`);
